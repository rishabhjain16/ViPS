...
def process_batch(batch_data, evaluator, mafi_calculator=None, include_additional_metrics=True, device=None):
    ...
    for _, row in batch_data.iterrows():
        ...
        try:
            ...
            row_result = {
                'std_viseme_score': result['standard']['viseme_score'],
                'wgt_viseme_score': result['weighted']['phonetically_weighted_viseme_score'],
                'std_phonetic_score': result['standard']['phonetic_alignment_score'],
                'wgt_phonetic_score': result['weighted']['phonetic_alignment_score']
            }
            ...
            if include_additional_metrics:
                ...
                try:
                    ...
                    row_result.update(wer_cer_results)
                except Exception:
                    pass
                ...
                try:
                    ...
                    if per_example_metrics and len(per_example_metrics) > 0:
                        for metric_name, value in per_example_metrics[0].items():
                            if metric_name not in ['word_error_rate', 'character_error_rate']:
...
def process_excel_with_metrics(excel_path, output_path=None, limit=None, include_additional_metrics=True, use_gpu=True, checkpoint_interval=1000, resume_from_checkpoint=False, num_processes=None):
