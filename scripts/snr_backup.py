...
class SNRMetricAnalyzer:
    ...
    def _create_correlation_heatmap(self, df, comparison_metrics, snr_labels):
        ...
        plt.title('Correlation between ViPS and Other Metrics Across SNR Levels')
        plt.tight_layout()
    ...
    def _create_r_squared_heatmap(self, df, comparison_metrics, snr_labels):
        ...
        plt.title('R² Values between ViPS and Other Metrics Across SNR Levels')
        plt.tight_layout()
    ...
    def _create_metric_line_plots(self, df, comparison_metrics, snr_labels):
        ...
        plt.plot(metric_means['snr_float'], metric_means['ViPS'], 'o-', 
                color='blue', label='ViPS', linewidth=2)
        
        # Plot other metrics
        colors = plt.cm.Set2(np.linspace(0, 1, len(comparison_metrics)))
        for (metric, title), color in zip(comparison_metrics, colors):
            plt.plot(metric_means['snr_float'], metric_means[metric], 'o-',
                    color=color, label=title, linewidth=2)
        
        plt.title('Average Metric Values Across SNR Levels')
    ...
    def _create_combined_analysis_plot(self, df, comparison_metrics, snr_labels):
        ...
        ax1.set_title('Correlation with ViPS')
        ...
        ax2.set_title('R² Values with ViPS')
