...
def create_correlation_plots(df, output_path='correlation_plots.png'):
    ...
    for idx, metric in enumerate(comparison_metrics):
        ...
        pearson_corr, p_value = stats.pearsonr(df['vips_score'], df[metric])
        ...
        # Create scatter plot
        ax.scatter(df['vips_score'], df[metric], alpha=0.6, color=colors[idx], s=40)
        
        # Add correlation line
        z = np.polyfit(df['vips_score'], df[metric], 1)
        p = np.poly1d(z)
        x_range = np.linspace(df['vips_score'].min(), df['vips_score'].max(), 100)
...
def create_combined_correlation_plots(df, output_path='combined_correlation_plots.png'):
    ...
    table_data.append(['ViPS', f"{df['vips_score'].mean():.3f}"])
    ...
    for idx, metric in enumerate(comparison_metrics):
        ...
        # Create scatter plot
        ax.scatter(df['vips_score'], df[metric], 
                  alpha=0.7,
                  color=colors[idx], 
                  s=60,
                  marker=markers[idx],
...
def create_mafi_combined_correlation_plots(df, output_path='mafi_combined_correlation_plots.png'):
    ...
    # Define metrics to compare against
    comparison_metrics = ['word_error_rate_avg', 'character_error_rate_avg', 'bertscore_f1_avg', 'semantic_similarity_avg', 'vips_score']
    metric_display_names = {
        'word_error_rate_avg': 'WER',
        'character_error_rate_avg': 'CER',
        'bertscore_f1_avg': 'BertScore',
...
def create_metrics_summary_table(df, output_path='metrics_summary.csv', markdown_path='metrics_summary.md'):
    ...
    metrics_dict = {
        ...
        'Mean': [df['vips_score'].mean()] + [df[m].mean() for m in comparison_metrics],
        'Std': [df['vips_score'].std()] + [df[m].std() for m in comparison_metrics],
    }
...
def create_correlation_comparison_plot(df, output_path='correlation_comparison_plot.png'):
    ...
    for metric in comparison_metrics:
        # ViPS correlation
        vips_corr, _ = stats.pearsonr(df['vips_score'], df[metric])
        vips_r2 = vips_corr ** 2
        # MaFI correlation
